...
const store = {
    ...
    set(key, val) { this._data[key] = Array.isArray(val) ? val : []; this._notify(key); },
...
async function fetchActive() {
    try { store.set('activeProcesses', await apiFetch('/active/processes')); } catch { return null; }
}

async function fetchActiveSessions() {
    try { store.set('activeSessions', await apiFetch('/active/sessions')); } catch { return null; }
}
...
async function fetchSessions() {
    try { store.set('sessions', await apiFetch('/store/sessions')); } catch { return null; }
}

async function fetchAutoWatch() {
    try { store.set('autoWatch', await apiFetch('/store/autowatch')); } catch { return null; }
}
...
async function addAutoWatch(name) {
    try {
        const r = await fetch(`${API}/store/autowatch`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name }),
        });
        if (r.status === 409) { toast('Already watching ' + name, 'error'); return; }
...
async function removeAutoWatch(name) {
    try {
        const r = await fetch(`${API}/store/autowatch/${encodeURIComponent(name)}`, { method: 'DELETE' });
        if (!r.ok) throw new Error();
        await fetchAutoWatch();
        renderDashboardWatchlist();
...
function startPoll() {
    ...
    pollTimer = setInterval(async () => {
        ...
        // Refresh detail panel if open (process mode only)
...
function weekSessions() {
    ...
    return store.get('sessions').filter(s => new Date(s.start_time) >= cutoff);
}

function computeStats() {
    const ws = weekSessions();
    const total = ws.reduce((a, s) => a + (s.duration || 0), 0);
    const today = ws.filter(s => isToday(s.start_time)).reduce((a, s) => a + (s.duration || 0), 0);
    ...
    return { total, today, avg, bestDay: DAYS[bestDay], bestDayFull: DAYS_FULL[bestDay], numDays };
}
...
function renderDashboardBuilding() {
    ...
    const totals = days.map(d => Object.values(d.procs).reduce((a, v) => a + v, 0));
...
function openSessionDetail(session) {
    ...
    let html = '<div class="detail-section">' +
        ...
        '<div class="detail-row"><span class="detail-label">Duration</span><span class="detail-value">' + fmtDur(dur) + '</span></div>' +
    ...
    if (p) {
        html += '<div class="detail-section">' +
            ...
            '<div class="detail-row"><span class="detail-label">Device</span><span class="detail-value">' + esc(p.device_name || 'Local') + '</span></div>' +
            '<div class="detail-row"><span class="detail-label">OS</span><span class="detail-value">' + esc(p.os || 'Unknown') + '</span></div>' +
            '<div class="detail-row"><span class="detail-label">Process Start</span><span class="detail-value">' + fmtTime(p.start_time) + '</span></div>' +
            '<div class="detail-row"><span class="detail-label">Is Parent</span><span class="detail-value">' + (p.is_parent ? 'Yes' : 'No') + '</span></div>' +
            '</div>';
    }
...
(async function init() {
    console.log('trkr: init start');
    try {
        console.log('trkr: fetching data');
        await fetchAll();
        console.log('trkr: data fetched, store=', JSON.stringify(Object.fromEntries(Object.entries(store._data).map(([k,v]) => [k, Array.isArray(v) ? v.length : v]))));
        console.log('trkr: rendering dashboard');
        renderDashboard();
        console.log('trkr: initializing router');
        initRouter();
        console.log('trkr: starting poll');
        startPoll();
        console.log('trkr: init complete');
    } catch (e) {
        console.error('trkr init error:', e);
